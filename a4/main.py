    ...
    for cam_idx, camera in enumerate(cameras):
        ...
        with torch.no_grad():
            ...
            if lights is not None:
                light_dir = light_location #TODO: Use light location and origin to compute light direction
...
def train_points(
    ...
    for epoch in range(0, cfg.training.num_epochs):
        ...
        for idx in t_range:
            ...
            loss = torch.abs(distances).mean() # TODO (Q2): Point cloud SDF loss on distances
            # loss = torch.exp(-1e2 * torch.abs(distances)).mean() # TODO (Q2): Point cloud SDF loss on distances
...
def pretrain_sdf(
    ...
    for iter in range(0, cfg.training.pretrain_iters):
        ...
        # distances, _ = model.implicit_fn.get_distance_color(points)
...
def train_images(
    ...
    for epoch in range(start_epoch, cfg.training.num_epochs):
        ...
        if (
            ...
            imageio.mimsave(cfg.fname1, [np.uint8(im * 255) for im in test_images])
            ...
            try:
                ...
                imageio.mimsave(cfg.fname2, [np.uint8(im * 255) for im in test_images])
            except Exception as e:
                print("Empty mesh")
                pass
