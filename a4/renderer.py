...
class SphereTracingRenderer(torch.nn.Module):
    ...
    def sphere_tracing(
        ...
        pts = origins
        for i in range(30):
            dist = implicit_fn(pts)
            pts = pts + dist*directions
...
def sdf_to_density(signed_distance, alpha, beta, type_):
    ...
    # s = 74
    # exp = torch.exp(-s*signed_distance)
    # density = (s*exp)/(1+exp)**2
...
class VolumeSDFRenderer(torch.nn.Module):
    ...
    def _aggregate(
        ...
        feature = (weights*rays_color).sum(axis=1)
        return feature
    ...
    def forward(
        ...
        for chunk_start in range(0, B, self._chunk_size):
            ...
            density = sdf_to_density(distance, self.alpha, self.beta, self.type) # TODO (Q3): convert SDF to density
