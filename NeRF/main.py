...
def render_images(
    ...
    for cam_idx, camera in enumerate(cameras):
        ...
        if cam_idx == 0 and file_prefix == '':
            ...
            imageio.imsave('images/q1_3_1.jpg', xy_vis)

        # TODO (1.3): Visualize rays using vis_rays
        if cam_idx == 0 and file_prefix == '':            
            ray_vis = vis_rays(ray_bundle, image_size)
            imageio.imsave('images/q1_3_2.jpg', ray_vis)
        ...
        if cam_idx == 0 and file_prefix == '':
            pass
            # render_points('images/q1_4.jpg', ray_bundle.sample_points.reshape(1,-1,3))

        # TODO (1.5): Implement rendering in renderer.py
        out = model(ray_bundle)

        # print(out['depth'].view( image_size[1], image_size[0], 1).detach().cpu())
        ...
        if cam_idx == 2 and file_prefix == '':
            depth_map = out['depth'].view( image_size[1], image_size[0], 1).detach().cpu().numpy()
            imageio.imsave('images/q1_5.jpg', depth_map)
...
def train(
    ...
    for epoch in t_range:
        for iteration, batch in enumerate(train_dataloader):
            ...
            loss = ((out['feature'] - rgb_gt)**2).sum(axis=1).mean()
    ...
    for epoch in range(start_epoch, cfg.training.num_epochs):
        ...
        if (
            ...
            with torch.no_grad():
                ...
                # imageio.mimsave('images/part_4_1.gif', [np.uint8(im * 255) for im in test_images])
                # imageio.mimsave(cfg.filename, [np.uint8(im * 255) for im in test_images])
                imageio.mimsave(cfg.filename+str(epoch)+".gif", [np.uint8(im * 255) for im in test_images])
